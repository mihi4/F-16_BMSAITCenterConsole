...
// FIXXXME michi - implement zeroize() call wenn altimeter knob is pressed more than x milliseconds
...
//Global variables
  ...
  bool configmode=false; // if set to true, the rotary button controls brightness of the displays
...
void setup()
{
 ...
 //precompile pull messages
  ...
  {                                          
	...
	SendMessage(datenfeld[var].request,1);
  }
  SendMessage("SETUP DONE",1);
  delay(1000);
}
 ...
 for (byte x=0;x<VARIABLENANZAHL;x++)     //loop through the data container. For each entry, the respective output will be updated
  {
    ...
    switch (datenfeld[x].typ)
    {
	  ...
	  #ifdef AltimeterX27
        case 54: //altimeter needle driven by X-class stepper motor
          UpdateAltimeterX27(x);
          break;
      #endif
...
void PullRequest(byte var)
{
  if(SERIALCOM.available())  //make sure input buffer is empty   
  {
	...
	//if(debugmode){SendMessage(datenfeld[var].request,1);}
    byte x=0;
...
bool CheckForSysCommand(byte SysCmd)
{
  ...
  //SendMessage("SYSCOMMANDCHECKED",1);
  if (SysCmdFound==true)
  {
    Reset();
    return true;
  ...
  {	  
    ...
    if (state==3)
    {
      ...
      else
      {
          ...
          if (c==VAR_ENDE)  //the termination character arrived. Save the data.
          {
            ...
            if (neuer_wert.varNr<100)     //only compute the data if a valid data position is found (everything above 99 is invalid)
            {
				if (strcmp(datenfeld[neuer_wert.varNr].wert, neuer_wert.wert)!=0)  //check if the recieved data is different from the stored data
              {
				...
				//if(debugmode){SendMessage(datenfeld[neuer_wert.varNr].wert,1);}
              }
              lastInput=millis(); //store last transmission time
            }  
            state=0;
			//if(debugmode){SendMessage("inReadReply",1);}				
			//if(debugmode){SendMessage(neuer_wert.wert,1);}
          }
		...
		/*if(debugmode){SendMessage("Reply",1);}				
		if(debugmode){SendMessage(neuer_wert.wert,1);} */
      }
    }
  }
